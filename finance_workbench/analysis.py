...
def build_scaled_benchmark(
    *,
    ...
    """
    Scale a simulation to the actually purchased/observed OPB.

    This is suitable for "simulation vs actual" monitoring when the initial
    simulation was built on a larger OPB and the comparison needs a like-for-like
    benchmark. It does not assert that every cash-flow item scales perfectly;
    that remains an explicit analytical assumption.
    """
    ...
    steps.append(
        CalculationStep(
            ...
            unit="ratio",
            assumptions=(
                "The simulation is scaled proportionally to actual OPB for benchmark comparison.",
            ),
        )
    )
    ...
    for period, value in simulated_values_by_period.items():
        ...
        steps.append(
            CalculationStep(
                ...
                inputs={
                    "simulated_value": str(raw),
                    "scale_ratio_calculation_id": ratio_id,
                },
                result=str(result),
                unit=unit,
                assumptions=(
                    "Proportional scaling is a comparison benchmark, not a forecast guarantee.",
                ),
            )
        )
